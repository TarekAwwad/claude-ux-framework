...
Migration note (2026-07-21, tables module): D11, D12, D13, D14, and D17 moved
from references/dashboards.md to references/tables.md. IDs unchanged;
checks C21 and C23 still derive from them; dashboards.md keeps a pointer.

## T: Tables and data grids (target: references/tables.md)
...
| ID | Rule (imperative, one line) | Source | Agent failure? |
|----|------------------------------|--------|----------------|
| T1 | Before building a table, name which of the four jobs it serves (find or filter rows, compare rows, view or edit one record, act on rows) and let that choose its controls and layout. | findings-tables NN/g data-tables | yes: agents ship a generic static table without matching controls to the actual task |
| T2 | Order columns by importance to the reader, keep columns people compare side by side, and make the first column a human-readable identifier rather than a raw generated ID. | findings-tables NN/g data-tables, findings-tables Pencil and Paper | yes: agents emit columns in schema order and lead with an opaque numeric or UUID key |
| T3 | Put units and shared qualifiers in the column header instead of repeating them in every cell, and left-align identifier-like numbers (IDs, dates, postal or phone codes) as text rather than right-aligning them as quantities. | findings-tables Pencil and Paper, findings-tables Carbon, findings-tables Salesforce; extends D11 | yes: agents repeat the unit in each cell and right-align every digit column, including IDs and dates |
| T4 | Keep the header row visible during vertical scroll and freeze the leftmost identifier column once the table scrolls horizontally, so row and column labels stay in view. | findings-tables NN/g data-tables, findings-tables NN/g mobile-tables, findings-tables Pencil and Paper | yes: observed correct for the sticky header in 2026-07-20-tables-diag, but sources document scroll-away headers as a common failure and the build did not freeze the first column on horizontal scroll |
| T5 | For large datasets, page or virtualize rows instead of rendering the whole dataset into the DOM at once (paginate past roughly 50 rows, virtualize once the row count grows large). | findings-tables ag-Grid, findings-tables TanStack, findings-tables Polaris | yes: agents render every row into the DOM regardless of size (the 2026-07-20-tables-diag build rendered all 400 rows with no windowing) |
| T6 | Show a row's detail in a non-modal side panel or an expandable inline row that keeps the rest of the table visible, and reserve a modal or full page for the densest records so comparison context is not covered. | findings-tables NN/g data-tables, findings-tables Pencil and Paper, findings-tables Polaris | yes: agents pop a modal over the table or navigate away to a detail page, destroying the rows people were comparing |
| T7 | Signal which cells are editable (a cursor or affordance on hover), confirm each edit explicitly (Enter, click-away, or a Save control) with clear success feedback, and route high-stakes edits to a panel or modal rather than inline. | findings-tables Pencil and Paper, findings-tables Salesforce | yes: agents make cells editable with no affordance, no explicit save, or no success feedback |
| T8 | In grid cells, never carry status or severity by color alone or in a hover title only; give each status an accessible text label or an icon paired with text. | observed in diagnostic run 2026-07-20-tables-diag (diag-3); strengthens P14 | yes: observed here, the attention marker was a color-only dot whose meaning lived only in a hover title, and sources (P14) document color-only status as a common failure |
| T9 | Give a data-backed table loading and error states, not just the populated and empty ones, and prefer a skeleton over a spinner when the load is likely to be noticeable. | observed in diagnostic run 2026-07-20-tables-diag (diag-4); P3 lineage; skeleton from findings-tables Carbon, findings-tables Polaris | yes: observed here, the build had no loading or error path at all, only ideal, empty, and partial-data |
| T10 | Distinguish an empty table caused by no data yet from one caused by no rows matching the current filter, and offer a clear-filters action only when a filter is actually set. | observed in diagnostic run 2026-07-20-tables-diag (diag-5); P3/P18 lineage | yes: observed here, one unconditional "no match, clear filters" message covered both cases and offered a button that did nothing when no filter was set |
| T11 | Do not let native HTML5 input constraints (min, max, required, pattern) preempt the app's own inline validation; keep the designed error messages reachable for every invalid value. | observed in diagnostic run 2026-07-20-tables-diag (diag-6) | yes: observed here, a number input's min blocked submit with a native bubble so the app's styled out-of-range error never ran |
| T12 | After a sort, page change, or filter re-renders the table, restore focus to the control the user acted on instead of dropping it to the document body. | observed in diagnostic run 2026-07-20-tables-diag (diag-1) | yes: observed here, every interaction rebuilt the thead and pager innerHTML, destroying the focused control and losing keyboard place |
| T13 | When a row opens a detail panel, move focus into it; when the panel is a full-screen overlay, trap focus and mark the background inert; return focus to the triggering row on close. | observed in diagnostic run 2026-07-20-tables-diag (diag-2) | yes: observed here, opening the panel left focus on the body with no role=dialog, focus trap, background inert, or focus return |
| T14 | When a table scrolls horizontally on a narrow viewport, make the scroll container a keyboard-focusable labeled region (role=region, aria-labelledby to the caption, tabindex=0, visible focus) and signal that content continues off-screen. | findings-tables Roselli (via CSS-Tricks summary), findings-tables NN/g mobile-tables, findings-tables Smashing; extends P22 | yes: agents wrap a table in overflow:auto with no keyboard access to the scroll region and no cue that columns continue off-screen |
...
- Show the current sort state on the sorted column with a direction arrow and aria-sort (findings-tables Pencil and Paper, Carbon, Polaris): the diagnostic build already colored the active column, drew a direction arrow, and set aria-sort without prompting (2026-07-20-tables-diag got-right), so this clears the agent-default-failure filter.
- Provide a bulk-selection checkbox column with an action bar that appears on selection, shows a live count, and offers a page-scoped select-all (findings-tables NN/g data-tables, Carbon, Polaris): the diagnostic build got all of this right unprompted (count survived paging and filtering, indeterminate select-all worked, delete guarded by a counted confirmation), so it does not earn a rule.
- Let users hide, reorder, resize, and reset columns from a menu and remember the choice (findings-tables NN/g data-tables, Pencil and Paper): a power-user enhancement whose absence does not block any of the four core table jobs, so it fails the agent-default-failure filter, like the accelerators cut above.
